...
`
  ...
  .params(u =>
    u
      .combine('storageClass', ['uniform', 'storage'] as const)
      .combine('storageMode', ['read', 'read_write'] as const)
  ...
  .fn(async t => {
    run(
      ...
      /* prettier-ignore */ [
        ...
        {input: [F32Bits(kBit.f32.subnormal.positive.max)], expected: anyOf(F32Bits(kBit.f32.subnormal.positive.max), F32(0)) },
        {input: [F32Bits(kBit.f32.subnormal.positive.min)], expected: anyOf(F32Bits(kBit.f32.subnormal.positive.min), F32(0)) },
