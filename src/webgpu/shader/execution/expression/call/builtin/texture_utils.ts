...
import { align, clamp, hashU32, lerp, quantizeToF32 } from '../../../../../util/math';
import { createTextureFromTexelView } from '../../../../../util/texture';
import {
  kTexelRepresentationInfo,
  PerTexelComponent,
  TexelRepresentationInfo,
} from '../../../../../util/texture/texel_data';
  ...
  const generator = (coords: Required<GPUOrigin3DDict>): Readonly<PerTexelComponent<number>> => {
    ...
    for (const component of repl.componentOrder) {
      ...
      texel[component] = lerp(
        repl.numericRange!.finiteMin,
        repl.numericRange!.finiteMax,
        normalized
      );
    }
    return quantize(texel, repl);
  };
...
export function expected<T extends Dimensionality>(
  ...
  const repr = kTexelRepresentationInfo[texture.texels.format];
  const textureExtent = reifyExtent3D(texture.info.size);
  ...
  switch (call.builtin) {
    case 'textureSample': {
      ...
      for (const sample of samples) {
        ...
        const v = load(c);
        for (const component of repr.componentOrder) {
...
export async function checkResults<T extends Dimensionality>(
  device: GPUDevice,
  texture: Texture,
  sampler: GPUSamplerDescriptor,
  calls: TextureCall<T>[]
  ...
  for (let callIdx = 0; callIdx < calls.length; callIdx++) {
    ...
    const gULP = repr.bitsToULPFromZero(repr.numberToBits(got));
    const eULP = repr.bitsToULPFromZero(repr.numberToBits(expect));
    for (const component of repr.componentOrder) {
...
`);
        const expectedSamplePoints = [
          'expected:',
          ...(await identifySamplePoints(texture.info, (texels: TexelView) => {
            return Promise.resolve(expected(call, { texels, info: texture.info }, sampler));
          })),
        ];
        const gotSamplePoints = [
          ...
          ...(await identifySamplePoints(
            ...
            async (texels: TexelView) =>
              (await doTextureCalls(device, { texels, info: texture.info }, sampler, [call]))[0]
          )),
        ];
...
export async function identifySamplePoints(
  ...
  while (unclassifiedStack.length > 0) {
    ...
    if (repl.componentOrder.some(c => results[c] !== 0)) {
  ...
  orderedTexelIndices.forEach((texelIdx, i) => {
    ...
    const w = repl.componentOrder.map(c => `${c}: ${weights[c]?.toFixed(5)}`).join(', ');
...
export type SamplePointMethods = 'texel-centre' | 'spiral';
export const kSamplePointMethods: SamplePointMethods[] = ['texel-centre', 'spiral'];
...
`,
  ...
  const gpuTexture = createTextureFromTexelView(device, texture.texels, texture.info);
